...
- **Smarter cover-art matching.** Roman/arabic numerals are treated as equal and glued letter/digit runs are split, so messy folder names resolve to the right Steam title (e.g. "Civilization 6" → *Sid Meier's Civilization VI*). The negative-result cache is now versioned and wiped when the matcher improves, so previously-unresolved games get another chance without re-downloading art that already worked.
- **Emulator logos by install path.** A "PS3 Emulator" entry living under `…\RPCS3` (and other stand-alone emulators) now gets its logo even when the display name doesn't name the emulator.
...
- Updated the **OptiScaler pre-SR multipass** fork (wilsjo2) to **0.7.6**: fixes the optional exposure/reactive-resource handoff in the DX11/Vulkan bridges, hardens NGX feature identification and release handling, and restores RR placement controls when the separate SR-only deferred option is selected. Adds opt-in RTX 20/30 MFG integration upstream (the optional external runtime is not bundled). The `RunBeforeSR` / `Passes` / `Precision` keys Adas writes are unchanged.
- Added an optional **Launch NeuralScreen** tool (perseval-BLR **1.5.1**, MIT). NeuralScreen is a standalone **whole-desktop** Neural Rendering overlay for RTX 30/40/50 — not a per-game route. Adas downloads the author's pinned 226 MB release archive at run time (SHA-256 verified), extracts it into the per-user tool cache, and launches `NeuralScreen.exe`; it is never bundled (the archive carries its own Python runtime and NVIDIA's leaked pre-release `nvngx_dlssnr.dll`, which Adas must not redistribute). Do not run it in competitive online games.
- Updated the verified **OneClick** helper launch to **0.13.11** (URL + SHA-256 re-pinned).
- **MFG Ada Unlock** now deploys upstream **0.8** automatically (it queries the latest release rather than pinning). 0.8 makes guarded Streamline UI/HUD composition the default, adds a validated Blackwell framework-kernel path for 3x/4x moving-detail stability, and adds NVIDIA Dynamic MFG integration for DLSS-G 310.9.1. No Adas code change was required.
- Refreshed the `tools/build-adas.ps1` payload manifest hashes for the AIO 2.2.1 files and the wilsjo2 0.7.6 archive.
