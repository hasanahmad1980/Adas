...
| Vulkan without native DLSS | Feeder | Imports shared D3D12 textures, fences, and semaphores into the game's Vulkan device. The 32-bit Vulkan route requires the matched 0.15.1 set. |
...
| 32-bit DirectX 10 | Native Feeder relay | Feeder 0.15.1 creates a private D3D11 relay and installs normally through `dxgi.dll`; no DXVK or machine-wide Vulkan layer is needed. |
...
> The ReShade suite and OptiScaler hook routes must not be installed together. Stable Feeder 0.7 also requires NVIDIA Smooth Motion to be disabled. The separately labelled 0.15.1 test route includes the current Smooth Motion synchronization fixes, but remains an upstream test build.
...
The **Standalone AIO 2.2.1** profile keeps NR, DLAA/SR and optional frame generation in one presentation pipeline. Upstream 2.2.0 makes NVIDIA Optical Flow the default motion source (pipelined ahead of Neural Rendering on D3D11/D3D12/Vulkan), which reduces temporal boiling, smearing and ghosting on 2x/3x NR and falls back to VORT or zero-motion guides when unavailable. The 2.2.1 maintenance release makes live resolution changes safer — quiescing the presentation worker before backbuffer recreation, cancelling a stale proxy present prepared before a resize, fixing a null-pointer crash when an overlay such as Discord intercepts the detached Present during a D3D12 resolution change, and pausing Frame Generation pacing while the ReShade menu is open. Adas defaults to DLSS Preset L, automatic window virtualization and telemetry, while leaving frame generation, VORT guidance and serialized presentation off. Hold F8 during launch to request the add-on's serialized safe-start recovery after a bad session. The adaptive GPU pressure governor that 2.0.9 folds in from the 2.0.7 experimental build, along with its source-resolution override, DPI virtualization correction, live color-profile switching and selectable neural-rendering pass counts, is available in-game but is not forced by Adas.
