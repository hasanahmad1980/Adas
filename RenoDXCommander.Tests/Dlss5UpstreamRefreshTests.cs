...
public sealed class Dlss5UpstreamRefreshTests
{
    ...
    {
        Assert.Equal("1.16.0-beta.3", Dlss5ComponentService.BundledFeederBetaVersion);
        Assert.Equal("1.0.5", Dlss5ComponentService.OpenGlBridgeVersion);
        Assert.Equal("0.13.19", Dlss5ComponentService.OneClickVersion);
    ...
    [Fact]
    public void BundledFeederBetaPayloadIsTheMatched1160Beta3Set()
    {
        var assetRoot = Path.Combine(AppContext.BaseDirectory, "Assets", "DLSS5");
        var expected = new[]
        {
            "dlss5-feed-1.16.0-beta.3.addon64",
            "dlss5-feed-1.16.0-beta.3.addon32",
            "dlss5-feed-host64-1.16.0-beta.3.exe",
            "DLSS5_Feed-1.16.0-beta.3.fx",
            "feed-vk-layer-1.16.0-beta.3-x64.zip",
            "feed-vk-layer-1.16.0-beta.3-x86.zip",
        };
    ...
    [Fact]
    public void PreSrMultipass08NoLongerRequiresTheHelperDllButOtherForksDo()
    {
        Assert.DoesNotContain("nvngx.dll_dlssnr.dll",
            Dlss5ComponentService.OptiScalerNrRequiredFiles("dxgi.dll", Dlss5InstallProfile.OptiScalerPreSrMultipass));
        Assert.Contains("nvngx.dll_dlssnr.dll",
            Dlss5ComponentService.OptiScalerNrRequiredFiles("dxgi.dll", Dlss5InstallProfile.OptiScalerNeuralRendering));
        Assert.Contains("nvngx.dll_dlssnr.dll",
            Dlss5ComponentService.OptiScalerNrRequiredFiles("dxgi.dll", Dlss5InstallProfile.OptiScalerNrBeforeSr));
    }

    [Fact]
    public void UpgradePrunesOnlyUnmodifiedFilesTheNewPackageNoLongerShips()
    {
        var root = Directory.CreateTempSubdirectory("adas-opti-prune-").FullName;
        try
        {
            ...
            var helper = Write("nvngx.dll_dlssnr.dll", "old helper");
            var modified = Write(Path.Combine("OptiScaler", "nvfp4", "kernel.bin"), "original");
            var kept = Write("dxgi.dll", "proxy");
            var ini = Write("OptiScaler.ini", "[DlssNr]");
            var record = new Dlss5InstallRecord();
            foreach (var path in new[] { helper, modified, kept, ini })
                record.InstalledHashes[path] = FileHelper.ComputeSha256(path);
            File.WriteAllText(modified, "user edit");

            Dlss5ComponentService.PruneSupersededOptiScalerNrFiles(root, record, new[] { kept });

            Assert.False(File.Exists(helper));
            Assert.False(record.InstalledHashes.ContainsKey(helper));
            Assert.True(File.Exists(modified)); // user-modified: never deleted
            Assert.True(File.Exists(kept));
            Assert.True(File.Exists(ini));
            Assert.True(record.InstalledHashes.ContainsKey(ini));
        }
        finally { Directory.Delete(root, true); }
    }
    ...
    [InlineData(Dlss5InstallProfile.LatestFeederBeta, Dlss5DeploymentMode.Dx11Feeder, "Feeder 1.16.0-beta.3", false)]
    ...
    [InlineData(Dlss5InstallProfile.StandaloneAio, Dlss5DeploymentMode.NativeDirectX12, "Standalone AIO 2.2.3", true)]
    [InlineData(Dlss5InstallProfile.StandaloneAio, Dlss5DeploymentMode.NativeDirectX12, "Standalone AIO 2.2.4", false)]
